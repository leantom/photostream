...
extension User: SnapshotParser {
    ...
    init(with snapshot: DataSnapshot, exception: String...) {
        ...
        if snapshot.hasChild("photoUrl") && !exception.contains("photoUrl") {
            avatarUrl = snapshot.childSnapshot(forPath: "photoUrl").value as! String
        }
        
        if snapshot.hasChild("deviceToken") && !exception.contains("deviceToken") {
            deviceToken = snapshot.childSnapshot(forPath: "deviceToken").value as! String
        }
    }
}
