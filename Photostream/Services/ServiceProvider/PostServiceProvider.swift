...
struct PostServiceProvider: PostService {
            ...
            return TransactionResult.success(withValue: data)
                ...
                followerRef.observeSingleEvent(of: .value, with: { followerSnapshot in
                    let postUpdate: [AnyHashable: Any] = [
                        ...
                        "likes_count": 0,
                        "comments_count": 0,
                        "avatar_url":self.session.user.avatarUrl
                    ]
    ...
    func like(id: String, callback: ((PostServiceError?) -> Void)?) {
        ...
        authorRef.observeSingleEvent(of: .value, with: { authorSnapshot in
            ...
            rootRef.child("users/\(authorId)").observeSingleEvent(of: .value, with: { (authorUser) in
                let user = User(with: authorUser, exception: "error get User")
                let notificationProvider = NotificationServiceProvider()
                notificationProvider.likePosts(from: user, token: user.deviceToken)
            })
