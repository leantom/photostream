...
extension PostDiscoveryViewController: PostListCollectionCellDelegate,AppImageViewerDelegate {
    func didTapCell(cell: PostListCollectionCell) {
        let appImage = ViewerImage.appImage(forImage: cell.photoImageView.image!)
        appImage.contentMode = .scaleAspectFit
        let viewer = AppImageViewer(originImage: cell.photoImageView.image!, photos: [appImage], animatedFromView: cell.photoImageView)
