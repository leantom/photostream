...
extension UserProfileViewV2: UserProfileViewConfig,UserProfileViewItemAction {
    func didTapToEdit() {
    ...
    fileprivate var userImage: UIImage {
        let frame = CGRect(x: 0, y: 0, width: avatar.frame.size.width, height: avatar.frame.size.width)
        let font = UIFont.systemFont(ofSize: avatar.frame.size.width / 2)
        let text: String = "A"
    ...
    func setupAvatar(with url: String) {
        let placeholder = avatar.image ?? userImage
    ...
    func configure(wiht item: UserProfileViewItem?, isProtoype: Bool = false) {
        ...
        lblNumberPosts.text = item.postCountText + " posts"
        lblNumberFollowers.text = item.followerCountText + " followers"
        lblNumberFollowing.text = item.followingCountText + " followings"
    ...
    fileprivate func setupActionButton(for type: ActionType) {
        ...
        btnFollow.removeTarget(self, action: #selector(didTapToFollow), for: .touchUpInside)
        btnFollow.removeTarget(self, action: #selector(didTapToUnfollow), for: .touchUpInside)
        ...
        btnFollow.addTarget(self, action: action, for: .touchUpInside)
    }
}
